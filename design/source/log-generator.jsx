...
// Streams events (ActivityManager, WindowManager, GC, Choreographer, etc.) plus app traffic.
...
const TAG_POOL = {
  "com.android.systemui": ["StatusBar", "NotifManager", "KeyguardUpdateMonitor", "BatteryController", "QSPanel"],
  "system_server": ["ActivityManager", "ActivityTaskManager", "WindowManager", "PowerManagerService", "AlarmManager", "JobScheduler", "PackageManager", "ConnectivityService"],
  "com.google.android.gms": ["GmsCore", "Auth", "Wearable", "Reachability"],
  "com.android.vending": ["Finsky", "PlayCore", "AssetModuleService"],
  "com.example.shopapp": ["MainActivity", "CartViewModel", "ApiClient", "ImageLoader", "Choreographer", "OkHttp", "Retrofit", "Glide", "Room"],
...
// weighted level picker
const LEVELS = ["V", "D", "I", "W", "E"];
const LEVEL_WEIGHTS = { V: 0.05, D: 0.45, I: 0.30, W: 0.13, E: 0.07 };
...
function hex(n = 4) { return Array.from({length: n}, () => "0123456789abcdef"[rint(0, 16)]).join(""); }

// Templated message generators per tag
const MSG_TEMPLATES = {
  ActivityManager: [
  ...
  BatteryController: [
    () => `Battery level: ${rint(20, 100)}%, charging=${Math.random() > 0.5}`,
  ],
  ...
  Glide: [
    () => `Load completed for [https://cdn.shop.example.com/p/${hex(8)}.webp] from [REMOTE]`,
  ],
  Room: [
  ...
  Retrofit: [
    () => `<-- HTTP 200 ${rint(40, 320)}ms`,
  ],

  // gms
  GmsCore: [
    () => `[GmsCore] Heartbeat to module ${pick(["AUTH", "FITNESS", "LOCATION", "DRIVE"])}`,
  ],
  Auth: [
    () => `Token refreshed for account ${hex(8)}@example.com`,
  ],
...
// occasional crash burst to trigger stack-trace pretty-print
const CRASH_BURST = (proc) => {
  const tag = "AndroidRuntime";
  const klass = pick(["NullPointerException", "IllegalStateException", "IndexOutOfBoundsException", "OutOfMemoryError"]);
  const cause = pick([
...
let _id = 0;
function makeEntry({ proc, tag, level, message, ts, isCrashLine }) {
  return {
    id: ++_id,
    ts: ts ?? Date.now(),
    pid: proc.pid,
    ...
    level,
    message,
    isCrashLine: !!isCrashLine,
  };
}

function generateOne(now = Date.now()) {
...
// Generates a batch with realistic clustering + occasional crash bursts.
// `intensity` bumps the count per tick.
function generateBatch(now, intensity = 1) {
  const out = [];
...
function seedHistory(seconds = 60, perSecond = 4) {
  const out = [];
  const now = Date.now();
...
window.LogGen = { generateOne, generateBatch, seedHistory, PROCESSES, TAG_POOL };
